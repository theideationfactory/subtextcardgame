...
export default function SpreadScreen() {
  ...
  const checkForExistingDraft = async (spreadType) => {
    try {
      ...
      // Get the most recent draft for the current user and spread type
      ...
      if (latestDraft?.draft_data?.zoneCards) {
        ...
        // Create a map of card IDs to card objects for quick lookup
        ...
        // Restore zone cards using the card map
        ...
        // Initialize empty zones for new spread
  ...
  const saveDraft = async (name?: string) => {
    ...
    try {
      ...
      // Convert zone cards to just IDs for storage
      const zoneCardIds = {};
